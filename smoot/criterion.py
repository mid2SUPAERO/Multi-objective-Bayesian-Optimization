...
class Criterion(object):
    def __init__(self, name, models, ref=None, s=None, hv =None, points = 300, random_state = None):
        self.models = models
        self.name = name
        self.ref = ref
        self.s = s
    ...
    def prob_of_feasability(x, const_modeles):
        ...
        x = np.asarray(x).reshape(1, -1)
        probs = [norm.cdf(-means[i](x)[0][0]/var[i](x)[0][0]) for i in range(len(const_modeles))]
